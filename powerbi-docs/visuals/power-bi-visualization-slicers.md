...
>[!TIP]
>목록 슬라이서 항목은 기본적으로 사전순 오름차순으로 정렬됩니다. 정렬 순서를 내림차순으로 되돌리려면 슬라이서의 오른쪽 위에 있는 추가 옵션(**...**)을 선택하고 드롭다운 목록에서 **구역 관리자별 정렬**을 선택합니다. 
...
1. 구역 관리자 슬라이서를 선택한 상태로:
    - Power BI Desktop에서 리본 메뉴의 **시각적 도구** 아래 **형식** 탭을 선택하고 **상호 작용 편집**을 선택합니다.
